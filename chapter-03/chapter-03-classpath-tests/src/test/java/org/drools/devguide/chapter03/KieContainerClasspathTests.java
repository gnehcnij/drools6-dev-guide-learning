...
public class KieContainerClasspathTests {
    ...
    public void loadingRulesFromClassPath() {
        ...
        results.getMessages().forEach((message) -> System.out.println(">> Message ( " + message.getLevel() + " ): " + message.getText()));
        assertThat(false, is(results.hasMessages(Message.Level.ERROR)));
        kContainer.getKieBaseNames()
                .stream()
                .peek((kieBase) -> System.out.println(">> Loading KieBase: " + kieBase))
                .forEach((kieBase) -> kContainer.getKieSessionNamesInKieBase(kieBase)
                        .forEach((kieSession) -> System.out.println("\t >> Containing KieSession: " + kieSession)));
    ...
    @Test
    public void loadingRules() {
        KieServices ks = KieServices.Factory.get();
        KieContainer kContainer = ks.newKieClasspathContainer();

        KieSession kieSession = kContainer.newKieSession("rules.cp.discount.session");
        kieSession.insert("666");
